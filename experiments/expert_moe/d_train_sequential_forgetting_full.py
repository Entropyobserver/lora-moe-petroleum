...
# Full fine-tuning requires a lower learning rate and more epochs than LoRA.
# Early stopping uses validation loss because running beam search over the full
# 600M model at every eval step is prohibitively slow.
LEARNING_RATE = 5e-5
NUM_EPOCHS = 15
...
def run_seed(seed, t):
    ...
    for step_idx, lang in enumerate(TRAIN_ORDER):
        ...
        # Load from the previous step's checkpoint to continue sequential training.
        # On the first step, load from the original pretrained backbone.
        torch.manual_seed(seed)
        model = AutoModelForSeq2SeqLM.from_pretrained(
            prev_model_path if prev_model_path is not None else BACKBONE,
        ...
        # Reload the saved checkpoint for evaluation to ensure we measure the
        # best checkpoint selected by early stopping, not the final iterate.
        torch.manual_seed(seed)
        ...
        # At the final step, evaluate all four language directions to give a
        # complete picture of forgetting across the full language set.
    ...
    # Forgetting is measured as the drop in EN--NO BLEU from step 1 to step 4.
