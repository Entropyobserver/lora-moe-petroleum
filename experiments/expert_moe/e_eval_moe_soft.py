...
RESULTS_ROOT = ROOT / "outputs" / "expert_moe" / "results" / "moe_soft"
...
def run_inference(moe, data, src_lang, max_samples):
    ...
    for sample in data[:max_samples]:
        ...
        with torch.no_grad():
            ...
            routing_log.append({"weights": weights.squeeze(0).tolist()})
            out = moe.generate(
                input_ids=inputs["input_ids"],
                attention_mask=inputs["attention_mask"],
...
def evaluate_seed(seed, max_samples, use_comet):
    ...
    for lang in LANGS:
        ...
        print(f"  moe_soft {lang} seed={seed}: inference done ({len(preds)} samples)", flush=True)
...
def main():
    ...
    for seed in args.seeds:
        print(f"\nEvaluating MoE soft routing  seed={seed}", flush=True)
